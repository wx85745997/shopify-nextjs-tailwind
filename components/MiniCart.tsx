...
import { CarContext, CarItem } from '@/context/shopContext'
import { formatter } from '@/utils/helpers'
import Link from 'next/link'

export default function MiniCart({ car }: { car: CarItem[] }) {
    const cancelButtonRef = useRef<HTMLButtonElement>(null)
    const { carOpen, setCarOpen, checkoutUrl, removeCartItem } = useContext(CarContext)
    let cartTotal = 0
    car.map((item) => {
        cartTotal += +item?.variantPrice * item?.variantQuantity
    })
    return (
        <Transition.Root show={carOpen} as={Fragment}>
            <Dialog
                as="div"
